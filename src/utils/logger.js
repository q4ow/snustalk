...
class Logger {
    ...
    rotateLogFiles() {
        ...
        if (this.currentFileSize >= this.options.maxFileSize) {
            const oldFiles = fs.readdirSync(this.options.logDir)
    ...
    log(level, message, ...args) {
        ...
        const formattedLog = this.formatter.format(LOG_LEVELS[level].label, formattedMessage, data);

        if (process.env.NODE_ENV !== 'test') {
            console.log(
                `${LOG_LEVELS[level].color}${formattedLog}${COLORS.reset}`
            );
        }
