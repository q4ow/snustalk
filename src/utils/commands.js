...
import { addNote, deleteNote, editNote, listNotes } from '../handlers/notesHandler.js';
...
export const commands = {
    ...
    'note': {
        ...
        async execute(message, args) {
            if (!args.length) {
                return message.reply('Usage: `note <add|list|delete|edit> [content|id]`');
            }
            ...
            try {
                switch (subcommand) {
                    ...
                    case 'edit':
                        ...
                        if (!id || !newContent.length) return message.reply('Please provide both note ID and new content.');
                        await editNote(message.author.id, id, newContent.join(' '));
                        return message.reply('✅ Note edited successfully.');

                    default:
                        return message.reply('Invalid subcommand. Use `add`, `list`, `delete`, or `edit`.');
                }
            } catch (error) {
