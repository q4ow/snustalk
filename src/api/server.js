...
app.patch('/api/guilds/:guildId/settings', authenticate, async (req, res) => {
    try {
        ...
        if (!guild?.members.cache.get(req.user.discord_id)?.permissions.has("Administrator")) {
...
app.get('/api/guilds/:guildId/tickets', authenticate, async (req, res) => {
    try {
        ...
        let tickets;
        const isStaff = member.roles.cache.has(await db.getRoleId(guild.id, "staff")) ||
            member.roles.cache.has(await db.getRoleId(guild.id, "management"));

        if (isStaff) {
            tickets = await db.getGuildTickets(req.params.guildId);
        } else {
            tickets = await db.getGuildTickets(req.params.guildId).then(tickets =>
...
app.get('/api/guilds/:guildId/tickets/:ticketId/messages', authenticate, async (req, res) => {
    try {
        const guild = req.app.get('client').guilds.cache.get(req.params.guildId);
        const member = guild?.members.cache.get(req.user.discord_id);
        if (!member) {
...
app.post('/api/guilds/:guildId/tickets/:ticketId/close', authenticate, async (req, res) => {
    try {
        const guild = req.app.get('client').guilds.cache.get(req.params.guildId);
        const member = guild?.members.cache.get(req.user.discord_id);
        if (!member) {
        ...
        const ticket = await db.getTicketInfo(req.params.ticketId);
        if (!ticket) {
            const ticketByChannel = await db.getTicketInfoByChannel(req.params.ticketId);
            if (!ticketByChannel) {
                return res.status(404).json({ error: 'Ticket not found' });
            }
...
app.get('/api/guilds/:guildId/automod', authenticate, async (req, res) => {
    try {
        const guild = req.app.get('client').guilds.cache.get(req.params.guildId);
        if (!guild?.members.cache.get(req.user.discord_id)?.permissions.has("ManageGuild")) {
            return res.status(403).json({ error: 'Missing required permissions' });
        }
...
app.get('/api/guilds/:guildId/logs', authenticate, async (req, res) => {
    try {
        const guild = req.app.get('client').guilds.cache.get(req.params.guildId);
        if (!guild?.members.cache.get(req.user.discord_id)?.permissions.has("ViewAuditLog")) {
            return res.status(403).json({ error: 'Missing required permissions' });
        }
...
app.patch('/api/guilds/:guildId/logs/:logType', authenticate, async (req, res) => {
    try {
        const guild = req.app.get('client').guilds.cache.get(req.params.guildId);
        if (!guild?.members.cache.get(req.user.discord_id)?.permissions.has("ManageGuild")) {
            return res.status(403).json({ error: 'Missing required permissions' });
        ...
        await db.updateLoggingSettings(req.params.guildId, req.params.logType, req.body);
        res.json({ success: true });
    } catch (error) {
