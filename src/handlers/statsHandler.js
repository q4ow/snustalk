...
async function updateStats(guild) {
    try {
        const statsChannels = {
            ...
            totalTickets: await guild.channels.fetch(process.env.STATS_TOTAL_TICKETS_CHANNEL_ID),
            openTickets: await guild.channels.fetch(process.env.STATS_OPEN_TICKETS_CHANNEL_ID)
        };
        ...
        await Promise.all([
            ...
            statsChannels.totalTickets?.setName(`📊 Total Tickets: ${stats.totalTickets}`),
            statsChannels.openTickets?.setName(`🎫 Open Tickets: ${stats.openTickets}`)
        ]);
