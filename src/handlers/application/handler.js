...
export async function handleApplyCommand(interaction) {
  try {
    ...
    // Check if the channel is a valid text-based channel that supports permissions
    // Using type check instead of isTextBased() method for compatibility
    if (![0, 2, 4, 5, 10, 11, 12, 14, 15].includes(appChannel.type)) {
      await interaction.reply({
        content:
          "❌ The applications channel is not a valid text channel. Please contact an administrator.",
        flags: 64,
      });
      console.error("Application channel is not a valid guild text channel");
      return;
    }
