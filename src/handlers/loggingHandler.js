...
class LogHandler {
    ...
    async createLog(type, data) {
        ...
        try {
            switch (type) {
                ...
                case 'MESSAGE':
                case 'VOICE':
                    await this[`format${type}Log`](embed, data);
                    break;
                case 'MOD':
                    await this.formatModLog(embed, data);
                    break;
                case 'CHANNEL':
                case 'ROLE':
                case 'SERVER':
        ...
        switch (data.action) {
            case 'JOIN':
                embed.setTitle('👋 Member Joined')
                    ...
                    .addFields(
                        { name: 'Account Created', value: `<t:${Math.floor(data.member.user.createdTimestamp / 1000)}:R>`, inline: true },
            ...
            case 'LEAVE':
                embed.setTitle('🚶 Member Left')
                    ...
                    .addFields(
                        { name: 'Joined At', value: `<t:${Math.floor(data.member.joinedTimestamp / 1000)}:R>`, inline: true },
                        { name: 'Member ID', value: data.member.id, inline: true }
                    );
                break;
            case 'NICKNAME':
                embed.setTitle('📝 Nickname Changed')
                    .addFields(
                        ...
                        { name: 'Old Nickname', value: data.oldNick || 'None', inline: true },
                        { name: 'New Nickname', value: data.newNick || 'None', inline: true }
                    );
                break;
        }
    }
            ...
            case 'DELETE':
                embed.setTitle('🗑️ Message Deleted')
                    .addFields(
                        ...
                        { name: 'Content', value: data.message.content || 'No content (possibly embed or attachment)' }
                    );
                break;
            case 'EDIT':
    ...
    formatVoiceLog(embed, data) {
        switch (data.action) {
            case 'JOIN':
                embed.setTitle('🎙️ Member Joined Voice')
                    .addFields(
                        { name: 'Member', value: `${data.member}`, inline: true },
                        { name: 'Channel', value: `${data.channel}`, inline: true }
                    );
                break;
            case 'LEAVE':
                embed.setTitle('🎙️ Member Left Voice')
                    .addFields(
                        { name: 'Member', value: `${data.member}`, inline: true },
                        { name: 'Channel', value: `${data.channel}`, inline: true },
                ...
                break;
            case 'MOVE':
                embed.setTitle('🔄 Member Moved in Voice')
                    .addFields(
                        { name: 'Member', value: `${data.member}`, inline: true },
            ...
            case 'ROLE_UPDATE':
                embed.setTitle('📝 Role Updated')
                    .addFields(
                        ...
                        {
                            name: 'Changes', value: Object.entries(data.changes)
    ...
    async formatUserLog(embed, data) {
        switch (data.action) {
            ...
            case 'DISCRIMINATOR_CHANGE':
                embed.setTitle('Discriminator Changed')
                    .addFields(
                        { name: 'User', value: `${data.user}`, inline: true },
                        { name: 'Old Discriminator', value: data.oldDiscriminator, inline: true },
                        { name: 'New Discriminator', value: data.newDiscriminator, inline: true }
                    );
                break;
        }
    }
            ...
            case 'CREATE':
                embed.setTitle('Invite Created')
                    .addFields(
                        ...
                        { name: 'Max Uses', value: data.maxUses?.toString() || 'Unlimited', inline: true },
                        { name: 'Expires', value: data.expiresAt ? `<t:${Math.floor(data.expiresAt.getTime() / 1000)}:R>` : 'Never', inline: true }
                    );
                break;
            case 'DELETE':
            ...
            case 'CREATE':
                embed.setTitle('Thread Created')
                    .addFields(
                        ...
                        { name: 'Creator', value: `${data.creator}`, inline: true },
                        { name: 'Parent Channel', value: `${data.thread.parent}`, inline: true }
                    );
                break;
            case 'DELETE':
                ...
                break;
            case 'ARCHIVE':
                embed.setTitle(data.archived ? 'Thread Archived' : 'Thread Unarchived')
                    .addFields(
                        { name: 'Name', value: data.thread.name, inline: true },
                        { name: 'Parent Channel', value: `${data.thread.parent}`, inline: true },
