...
export async function handleSettingsCommand(interaction) {
    ...
    switch (subcommand) {
        case "set":
            await handleSetSetting(interaction, settingType, settingName, settingValue);
            break;
        case "get":
...
async function handleSetSetting(interaction, type, name, value) {
    try {
        ...
        switch (type) {
            ...
            default:
                await interaction.reply({
                    content: "Invalid setting type. Use 'channel', 'role', 'api', or 'link'.",
                    flags: 64
                });
                return;
        }
...
async function handleGetSetting(interaction, type, name) {
    try {
        ...
        await interaction.reply({
            content: value ? `${type}.${name} = ${value}` : `${type}.${name} is not set`,
            flags: 64
        });
    } catch (error) {
